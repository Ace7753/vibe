...
async def run_vibe_engine(job_id: str, query: str, base_url: str):
    ...
    # 1. SpotDL (Master)
    spotdl_cmd = [sys.executable, "-m", "spotdl", "download", query, "--output", str(DOWNLOAD_DIR), "--format", "m4a", "--threads", "4"]
...
async def start_dl(request: Request, query: str = Form(...)):
    ...
    JOBS[job_id] = {"id": job_id, "status": "queued", "log": ["Engine warming up..."], "zip_url": None}
...
<body class="min-h-screen pb-24">
    ...
    <main class="relative z-10 max-w-xl mx-auto pt-16 px-6">
        <header class="mb-12 flex justify-between items-start">
            <div class="flex items-center gap-4"><img src="/assets/vibe_icon_original.png" class="w-12 h-12 rounded-xl shadow-xl" alt="Logo"><div><h1 class="text-3xl font-black tracking-tighter">{c['title']}</h1><p class="text-xs font-bold opacity-40 uppercase">Multi-Engine V2</p></div></div>
