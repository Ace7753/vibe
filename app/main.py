...
async def try_engine(job, label, cmd):
    job["log"].append(f"🔍 [Chain] Swapping to {label}...")
    try:
        # We use a timeout to prevent an engine from hanging forever
        ...
        try:
            await asyncio.wait_for(read_stream(proc.stdout), timeout=300) # 5 min timeout per engine
...
async def run_vibe_engine(job_id: str, query: str, base_url: str):
    ...
    # Adding impersonation to SpotDL for better unblocking
    spotdl_cmd = [
        sys.executable, "-m", "spotdl", "download", query,
        "--output", str(DOWNLOAD_DIR), "--format", "m4a", "--threads", "4",
        "--yt-dlp-args", "--impersonate chrome --no-check-certificate --geo-bypass --rm-cache-dir --extractor-args \"youtube:player_client=android,ios,web;player_skip=webpage\""
    ]
    ...
    # --- ENGINE 3: VOTIFY ---
    if len({f.name for f in DOWNLOAD_DIR.glob("*")} - before) == 0:
        votify_cmd = ["votify", query, "-o", str(DOWNLOAD_DIR)]
        # Votify can use the same cookie file
        if COOKIE_FILE.exists(): votify_cmd.extend(["-c", str(COOKIE_FILE)])
        await try_engine(job, "Votify", votify_cmd)

    # --- ENGINE 4: ONTHESPOT ---
    if len({f.name for f in DOWNLOAD_DIR.glob("*")} - before) == 0:
        # Try running onthespot-cli directly
...
<body class="min-h-screen pb-24">
    ...
    <main class="relative z-10 max-w-xl mx-auto pt-16 px-6">
        <header class="mb-12 flex justify-between items-start">
            <div class="flex items-center gap-4"><img src="/assets/vibe_icon_original.png" class="w-12 h-12 rounded-xl shadow-xl" alt="Logo"><div><h1 class="text-3xl font-black tracking-tighter">{c['title']}</h1><p class="text-xs font-bold opacity-40 uppercase">God Mode Active</p></div></div>
        ...
        <section id="page-download" class="space-y-8">
            ...
            <div id="card" class="glass rounded-2xl p-8 space-y-4">
                ...
                <div id="logs" class="text-[9px] font-mono text-emerald-500/50 h-32 overflow-y-auto leading-tight p-4 bg-black/20 rounded-xl border border-white/5">Waiting...</div>
